...
export const NIGERIA_STATES = NIGERIA_STATES_LGAS.map(loc => loc.state)

export function getLGAsForState(state: string): string[] {
  const location = NIGERIA_STATES_LGAS.find(loc => loc.state === state)
  return location ? location.lgas : []
}
