...
export async function GET(req: NextRequest, ctx: Ctx) {
  try {
    ...
    const [session] = await withDbRetry(() => db
      .select({ id: chatSessions.id, userId: chatSessions.userId, guestEmail: chatSessions.guestEmail })
      .from(chatSessions)
    ...
    const messages = await withDbRetry(() => db
      .select()
      .from(chatMessages)
...
export async function POST(req: NextRequest, ctx: Ctx) {
  try {
    ...
    // Verify user owns the session or is admin
    const [session] = await withDbRetry(() => db
      .select({ id: chatSessions.id, userId: chatSessions.userId, guestEmail: chatSessions.guestEmail, status: chatSessions.status })
      .from(chatSessions)
      .where(eq(chatSessions.id, sessionId))
      .limit(1))
...
function canAccessSession(
  req: NextRequest,
  session: { id: string; userId: string | null; guestEmail: string | null },
  ...
  return Boolean(session.guestEmail && verifyGuestChatToken(session.id, session.guestEmail, token))
}
