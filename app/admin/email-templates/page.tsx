...
export default function EmailTemplatesPage() {
  ...
  return (
    <div className="max-w-6xl space-y-6 animate-page-reveal">
      <AdminPageHeader title="Email Templates" description="Preview the exact transactional layout customers receive. Sample data is used and no email is sent." />
      <div className="grid gap-6 lg:grid-cols-[220px_1fr]">
        <nav className="flex gap-2 overflow-x-auto lg:flex-col" aria-label="Email templates">
          {(Object.keys(EMAIL_PREVIEWS) as Array<keyof typeof EMAIL_PREVIEWS>).map(key => <button key={key} onClick={() => setSelected(key)} aria-current={selected === key ? 'page' : undefined} className={`min-h-10 shrink-0 rounded-lg px-3 py-2.5 text-left text-sm font-medium transition-colors focus-visible:ring-3 focus-visible:ring-ring/20 ${selected === key ? 'bg-primary text-primary-foreground' : 'border border-border bg-card hover:bg-muted'}`}>{LABELS[key]}</button>)}
        </nav>
